...
sudo cp platform/openhitls/build/*.so /usr/local/lib/
...
sudo cp src/scloudplus/include/*.h /usr/local/include/pqcp/
sudo cp src/provider/*.h /usr/local/include/pqcp/
sudo cp build/libpqcp_provider.so /usr/local/lib/
sudo mkdir -p /usr/local/include/hitls && \
find platform/openhitls/include -mindepth 1 -maxdepth 1 -type d -exec cp -r {} /usr/local/include/hitls/ \;
sudo ldconfig
